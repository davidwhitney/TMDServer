...
const throttlingClient = {
  nextCallDate: null,
  remainingCallsPerPeriod: null,
  throttledCall: { throttled: true },
  requestShouldBeThrottled: function() { return this.nextCallDate != null && Date.now() < this.nextCallDate },
  get: async function(timelineUri, timelineQuery) { 
    ...
    const result = await T.get(timelineUri, timelineQuery).catch(err => { });
    
    this.calculateNextCallTime(result);
    return result;
  },
  calculateNextCallTime: function(result) {
...
module.exports = async (context, req) => {
  ...
  const response = await throttlingClient.get('statuses/mentions_timeline', query);
